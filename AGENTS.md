...
71. Startup preflight and artifact validation binding that requires real Codex operator unlock requests to be stored as artifacts inside the current run directory.
72. A narrow iteration-loop startup exception that permits an existing run directory only when it contains the configured real Codex operator unlock request artifact; ordinary existing run directories remain blocked.
