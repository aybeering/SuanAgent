...
The active implementation may include the deterministic evaluation loop,
strategy modifier stubs, guarded external-adapter slots, proposal contracts,
artifact validation, replay commands, and Codex CLI readiness evidence. The
detailed component roadmap lives in `docs/contract_roadmap.md`.

Keep `AGENTS.md` focused on rules that affect day-to-day code changes. When a
new artifact family or readiness stage is added, update the roadmap and the
matching schema/tests instead of expanding this scope section into another long
checklist.
