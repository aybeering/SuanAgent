...
counts and consumes a deterministic local `chart.html` artifact, but visual-agent
execution remains disabled and cannot change routing or acceptance.
...
`chart.html`, `visual_review.json`, `visual_review.md`,
...
from before-trade files and before-metrics. It has no external scripts, assets,
or network calls. `visual_review.json` records the contract-only visual review
stub's view of before trade files and this chart artifact. The artifact
validator rejects the chart if it references external network assets, and
rejects `visual_review.json` if it claims visual-agent execution, routing
authority, or final acceptance authority while V0.5 keeps visual review
inactive.
