...
commit. It also checks the `summary.md` dataset, run-outcome, agent-intake, and
round-table sections against the corresponding manifest records and round
artifacts, so operator-facing data split, outcome, per-round, and agent-output
