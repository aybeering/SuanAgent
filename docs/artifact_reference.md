...
- `external_agent_sandbox_drill.json` audits external-slot command, workspace,
  ...
  and execution-audit artifact carries a file SHA-256 binding, so operator
  review can compare the exact external command, isolation policy, and saved
  execution evidence without invoking it. Saved sandbox-drill reports are
  strict snapshots and fail validation when current run-local boundary evidence
  no longer matches the saved drill.
