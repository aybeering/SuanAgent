...
artifact-health history status, record counts, replay manifest-drift
observation counts, latest replay-drift count, failed run ids, and the
read-only `health-history --markdown` review command. The command does not
rewrite `index.jsonl`, create
...
with the same command digest bindings, Codex preflight next-step hint, and
safety flags. These are read-only convenience fields and do not rewrite index
...
gate artifact-path binding, gate artifact file-record SHA-256 binding,
read-only authority, policy flags, selected-candidate count, candidate-quality
source-path binding, candidate-quality source file-record SHA-256 binding,
candidate-quality selected-count binding, candidate/selectable/selected count
bounds, plus research-watchlist source-path binding, research-watchlist source
file-record SHA-256 binding, and research-watchlist status and alert count
...
source-home payload snapshot SHA-256, source-home source-view set SHA-256,
Codex readiness summary fields including the preflight next step, authority
flags, command SHA-256 bindings, and read-only policy. Its markdown view also
displays the selector source marker, navigation summary, selected-command
digest, source-home command digest, source-home payload snapshot digest,
source-home source-view set digest, and
...
binding, source-home source-view set binding, safety flags, authority flags,
and read-only policy before falling back to the full derived-payload
comparison.
