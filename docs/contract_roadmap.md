...
129d. A read-only `python -m orchestrator.experiments show <run_id>` enhancement that returns the same derived `operator_home` and `operator_next_command` hints in the compact run payload: iteration-loop runs expose the terminal-only home markdown command, next-command selector command, command SHA-256 bindings, status, read-only boundary, hint-only flags, next-command text, label, status, blocked flag, blocker count, operator hint, boundary, write target, explicit-invocation flag, approval flags, guarded-executor flag, and hint-only flag, while single-run payloads mark the hints and next-command state unavailable, without rewriting `index.jsonl`, creating artifacts, executing commands, writing config, promoting champions, executing agents, rerunning backtests, routing candidates, applying patches, or changing acceptance.
