...
129b-3. A `run_diagnosis_operator_navigation_v1` block in saved and terminal-only `diagnosis.json` output from `python -m orchestrator.experiments diagnose <run_id>`, plus a terminal-only `diagnose <run_id> --markdown` view, derived from the final manifest `operator_home` row for iteration runs, that surfaces the read-only home command, next-command selector command, source home command, selected command, blocked state, blocker count, first blocker, boundary, write target, command SHA-256 bindings, safety flags, and operator hint next to deterministic run outcome and artifact-health diagnosis, while marking single-run or missing-home diagnoses unavailable, with artifact-validator consistency checks that bind diagnosis navigation back to `manifest.operator_home`, source-home command digests, selected-command digests, blocker identity, and read-only policy flags, without creating artifacts, recording approval, executing commands, writing config, promoting champions, executing agents, rerunning backtests, routing candidates, applying patches, or changing acceptance.
