...
61. Deterministic `agent_execution.json` startup-preflight binding that lets artifact validation prove guarded Codex CLI attempts used the command digest reviewed by startup preflight and stayed under the current run workspace prefix plus strategy-only mutation allowlist and mutation guard.
