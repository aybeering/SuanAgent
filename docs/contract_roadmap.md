...
120. Deterministic operator-cockpit binding for the saved Codex CLI execution readiness diff, including source-file hash validation, summary counters, a dedicated `codex_cli_readiness_diff` panel, a `codex_cli_intake` panel for selected-attempt intake-binding status, and a read-only command hint for reviewing the diff, without recording approval, executing commands, executing Codex, creating workspaces, writing config, executing agents, rerunning backtests, routing candidates, applying patches, or changing acceptance.
