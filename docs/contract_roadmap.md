...
129b-2. A derived `operator_next_command` hint on `python -m orchestrator.experiments list --limit N`, `python -m orchestrator.experiments show <run_id>`, and `python -m orchestrator.experiments summary`, including an `experiment_operator_next_command_entry_v1` object in `schemas/experiment_summary_dashboard.schema.json`, that surfaces the read-only `next-command <run_id> --markdown` selector, selected command, selected status, blocked state, blocker count, boundary, write target, safety flags, and operator hint copied from the operator-home next-command fields, while marking single-run or missing-run rows unavailable, without rewriting `index.jsonl`, creating artifacts, executing commands, writing config, promoting champions, executing agents, rerunning backtests, routing candidates, applying patches, or changing acceptance.
129b-3. A `run_diagnosis_operator_navigation_v1` block in saved and terminal-only `diagnosis.json` output from `python -m orchestrator.experiments diagnose <run_id>`, derived from the final manifest `operator_home` row for iteration runs, that surfaces the read-only home command, next-command selector command, selected command, blocked state, blocker count, boundary, write target, safety flags, and operator hint next to deterministic run outcome and artifact-health diagnosis, while marking single-run or missing-home diagnoses unavailable, with artifact-validator consistency checks that bind diagnosis navigation back to `manifest.operator_home` and read-only policy flags, without creating artifacts, recording approval, executing commands, writing config, promoting champions, executing agents, rerunning backtests, routing candidates, applying patches, or changing acceptance.
