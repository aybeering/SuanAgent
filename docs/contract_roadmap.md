...
42. A deterministic `external_agent_sandbox_drill.json` report that audits external slot command, command SHA-256 binding, workspace manifest SHA-256 binding, execution-audit SHA-256 binding, input artifact and bundle SHA-256 bindings, output, subprocess, and mutation-guard evidence without executing agents.
