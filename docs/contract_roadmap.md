...
124. Strict local consistency validation for the terminal-only `operator_run_review_v1` payload exposed by `python -m orchestrator.experiments review <run_id>`, including run-dir and closeout-path binding, embedded dashboard fixed gate order, gate-to-summary status bindings, selected-candidate and candidate-quality selected-count bindings, candidate/selectable/selected count bounds, research-watchlist status and alert-count bindings, read-only dashboard authority, and policy flag equality between the review wrapper and dashboard, applied before JSON or markdown output is printed, without writing config, promoting champions, executing agents, rerunning backtests, routing candidates, applying patches, or changing acceptance.
