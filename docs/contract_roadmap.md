...
129b-2. A derived `operator_next_command` hint on `python -m orchestrator.experiments list --limit N`, `python -m orchestrator.experiments show <run_id>`, and `python -m orchestrator.experiments summary`, including an `experiment_operator_next_command_entry_v1` object in `schemas/experiment_summary_dashboard.schema.json`, that surfaces the read-only `next-command <run_id> --markdown` selector, source home command, selected command, selected status, blocked state, blocker count, boundary, write target, command SHA-256 bindings, safety flags, and operator hint copied from the operator-home next-command fields, while marking single-run or missing-run rows unavailable, without rewriting `index.jsonl`, creating artifacts, executing commands, writing config, promoting champions, executing agents, rerunning backtests, routing candidates, applying patches, or changing acceptance.
