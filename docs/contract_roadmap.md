...
136. A read-only `config_operator_runbook.json` and `config_operator_runbook.md` report pair, written by `python -m orchestrator.config_operator_runbook` and exposed through `python -m orchestrator.experiments config-runbook <run_id>`, that orders the config candidate, operator review, application dry-run, guarded apply, rollback preview, guarded restore, and lineage chain into one operator guide with artifact file records, step statuses, blockers, and command hints that are checked for shell-control-token safety, explicit operator invocation, write-config marking, summary-count consistency, authority flags, and read-only policy before JSON or markdown output is printed, without recording approval, executing commands, writing config, restoring config, deleting memory, executing agents, rerunning backtests, routing candidates, applying patches, or changing acceptance.
