...
def validate_iteration_diagnosis_operator_navigation(
    *,
    ...
    if str(next_command.get("source_home_command", "")) != str(home.get("command", "")):
        add_error(
            report,
            "diagnosis.json operator_navigation source home command mismatch",
        )
    if str(next_command.get("source_home_command_sha256", "")) != str(
        home.get("command_sha256", "")
    ):
        add_error(
            report,
            "diagnosis.json operator_navigation source home sha256 mismatch",
        )
    if str(next_command.get("source_home_command_sha256", "")) != sha256_text(
        str(next_command.get("source_home_command", ""))
    ):
        add_error(
            report,
...
def validate_unavailable_diagnosis_operator_navigation(
    *,
    ...
    if str(next_command.get("selector_boundary", "")):
        add_error(report, "diagnosis.json operator_navigation selector boundary mismatch")
    if str(next_command.get("source_home_command", "")):
        add_error(
            report,
            "diagnosis.json operator_navigation source home command mismatch",
        )
    if str(next_command.get("source_home_command_sha256", "")):
        add_error(
            report,
            "diagnosis.json operator_navigation source home command_sha256 mismatch",
        )
...
def validate_optional_external_agent_sandbox_drill(
    *,
    ...
    for slot in slots:
        ...
        execution_audit = slot.get("execution_audit", {})
        if not isinstance(execution_audit, dict):
            add_error(report, "external_agent_sandbox_drill.json execution_audit invalid")
            continue
        execution_path = str(execution_audit.get("path", ""))
        resolved_execution_path = (
            Path(execution_path)
            if Path(execution_path).is_absolute()
            else repo_root / execution_path
        )
        expected_execution_sha256 = (
            hashlib.sha256(resolved_execution_path.read_bytes()).hexdigest()
            if execution_path
            and resolved_execution_path.exists()
            and resolved_execution_path.is_file()
            else ""
        )
        if execution_audit.get("artifact_sha256") != expected_execution_sha256:
            add_error(
                report,
...
def validate_optional_codex_cli_enablement_gate(
    *,
    ...
    for error in validate_codex_cli_enablement_gate_file(
        payload_path=path,
        repo_root=repo_root,
        schema_path=repo_root / "schemas/codex_cli_enablement_gate.schema.json",
    ):
        add_error(report, f"codex_cli_enablement_gate.json file: {error}")
    payload = validate_json_object(path=path, report=report)
    if payload is None:
        return
    if payload.get("run_id") != report.get("run_id"):
...
def validate_optional_codex_cli_canary_gate(
    *,
    ...
    if not path.exists():
        return
    checked_files(report).append(str(path))
    from orchestrator.codex_cli_canary_gate import validate_codex_cli_canary_gate_file

    for error in validate_codex_cli_canary_gate_file(
        payload_path=path,
        repo_root=repo_root,
        schema_path=repo_root / "schemas/codex_cli_canary_gate.schema.json",
    ):
        add_error(report, f"codex_cli_canary_gate.json file: {error}")
    payload = validate_json_object(path=path, report=report)
    if payload is None:
        return
    if payload.get("run_id") != report.get("run_id"):
...
def validate_optional_codex_cli_real_preflight(
    *,
    ...
    for error in validate_codex_cli_real_preflight_file(
        payload_path=path,
        repo_root=repo_root,
        schema_path=repo_root / "schemas/codex_cli_real_preflight.schema.json",
    ):
        add_error(report, f"codex_cli_real_preflight.json file: {error}")
    payload = validate_json_object(path=path, report=report)
    if payload is None:
        return
    if payload.get("run_id") != report.get("run_id"):
...
def validate_optional_codex_cli_dry_invocation_guard(
    *,
    ...
    for error in validate_codex_cli_dry_invocation_guard_file(
        payload_path=path,
        repo_root=repo_root,
        schema_path=repo_root / "schemas/codex_cli_dry_invocation_guard.schema.json",
    ):
        add_error(report, f"codex_cli_dry_invocation_guard.json file: {error}")
    payload = validate_json_object(path=path, report=report)
    if payload is None:
        return
    if payload.get("run_id") != report.get("run_id"):
...
def validate_optional_codex_cli_execution_unlock_gate(
    *,
    ...
    for error in validate_codex_cli_execution_unlock_gate_file(
        payload_path=path,
        repo_root=repo_root,
        schema_path=repo_root / "schemas/codex_cli_execution_unlock_gate.schema.json",
    ):
        add_error(report, f"codex_cli_execution_unlock_gate.json file: {error}")
    payload = validate_json_object(path=path, report=report)
    if payload is None:
        return
    if payload.get("run_id") != report.get("run_id"):
    ...
    if not isinstance(policy, dict):
        ...
        for key in (
            ...
            "requires_canary_preflight_binding",
...
def validate_optional_codex_cli_real_execution_dry_run(
    *,
    ...
    for error in validate_codex_cli_real_execution_dry_run_file(
        payload_path=path,
        repo_root=repo_root,
        schema_path=repo_root
        / "schemas/codex_cli_real_execution_dry_run.schema.json",
    ):
        add_error(report, f"codex_cli_real_execution_dry_run.json file: {error}")
    payload = validate_json_object(path=path, report=report)
    if payload is None:
        return
    if payload.get("run_id") != report.get("run_id"):
