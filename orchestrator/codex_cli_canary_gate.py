...
SCHEMA_PATH = Path("schemas/codex_cli_canary_gate.schema.json")
...
def validate_codex_cli_canary_gate_file(
    *,
    payload_path: Path,
    repo_root: Path = Path("."),
    ...
    """Validate a saved canary gate against schema and current evidence."""
    repo_root = repo_root.resolve()
    schema_errors = tuple(
        validate_json_file(
    ...
    if not run_dir_value:
        return schema_errors + ("codex_cli_canary_gate run_dir required",)
    if not config_path_value:
        return schema_errors + ("codex_cli_canary_gate config_path required",)
    ...
    if payload != expected:
        return schema_errors + ("codex_cli_canary_gate current evidence mismatch",)
    return schema_errors
