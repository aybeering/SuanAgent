...
from orchestrator.operator_action_approval import (
    ...
    REQUIRED_CONFIRMATION_PHRASE as OPERATOR_ACTION_CONFIRMATION_PHRASE,
...
def test_config_change_candidate_records_manual_config_edits(
    ...
    repo = copy_repo_fixture(tmp_path)
    run_dir = repo / "experiments/config-candidate"
    run_dir.mkdir(parents=True)
    memory_path = repo / "experiments/memory.jsonl"
        ...
        records.append(
            {
                "created_at": f"2026-01-{(index % 28) + 1:02d}T00:00:00Z",
                "kind": "proposal_outcome",
                "run_id": f"candidate-memory-run-{index:03d}",
                "round_id": "round_001",
                "agent_name": "stub",
                "direction_tag": "lower_min_edge",
                "patch_sha256": "configpatch" * 7,
                "accepted": False,
                "applicable": True,
                "validation_ev_delta": -0.01,
            }
    ...
    write_memory_hygiene(
        ...
        failed_patch_threshold=2,
        failed_direction_threshold=3,
        exclude_run_id="config-candidate",
    )
    write_memory_scope_recommendation(
        run_dir=run_dir,
        repo_root=repo,
    ...
    dynamic_payload = build_config_change_candidate(
        run_dir=run_dir,
        repo_root=repo,
        experiments_dir=repo / "experiments",
    )

    assert payload["schema_version"] == CONFIG_CHANGE_CANDIDATE_SCHEMA_VERSION
    assert payload["summary"]["status"] == "changes_recommended"
    assert payload["summary"]["candidate_count"] == 1
    assert payload["changes"][0]["config_path"] == "memory_filter.recent_record_limit"
    assert payload["changes"][0]["operation"] == "set"
    assert payload["changes"][0]["current_value"] == 0
    assert payload["changes"][0]["proposed_value"] == 100
    assert payload["changes"][0]["applied"] is False
    assert payload["changes"][0]["requires_operator_review"] is True
    assert payload["policy"]["does_not_write_config"] is True
    assert payload["sources"][0]["file"]["path"].endswith(
        "memory_scope_recommendation.json"
    )
    assert dynamic_payload["sources"][0]["from_artifact"] is True
...
def test_operator_action_approval_records_intent_without_executing_command(
    tmp_path: Path,
) -> None:
    repo = copy_repo_fixture(tmp_path)
    run_iteration_loop(
    ...
    run_dir = repo / "experiments/operator-action-approval"
    strategy_before = (repo / "strategies/current_strategy.py").read_text(
        encoding="utf-8"
    )
    ...
    preview = build_operator_action_approval(
        run_dir=run_dir,
        repo_root=repo,
        action_id=action["action_id"],
        command_label=command["label"],
    )
    assert preview["schema_version"] == OPERATOR_ACTION_APPROVAL_SCHEMA_VERSION
    assert preview["status"] == "ready_for_operator_approval"
    assert preview["operator_intent"]["approval_recorded"] is False
    assert "operator_approval_not_recorded" in preview["approval_gate"][
        "approval_blockers"
    ]
    assert preview["selected_command"]["label"] == command["label"]
    ...
    assert blocked["operator_intent"]["approval_recorded"] is False
    assert blocked["operator_intent"]["confirmation_phrase_matches"] is False
    assert "confirmation_phrase_mismatch" in blocked["approval_gate"][
        "approval_blockers"
    ]

    json_path, md_path, approval = write_operator_action_approval(
        run_dir=run_dir,
        repo_root=repo,
        operator_id="test-operator",
        action_id=action["action_id"],
    ...
    markdown = render_operator_action_approval_markdown(approval)

    assert approval["status"] == "approval_recorded"
    assert approval["operator_intent"]["approval_recorded"] is True
    assert approval["operator_intent"]["explicit_approval"] is True
    assert approval["operator_intent"]["operator_id"] == "test-operator"
    assert approval["operator_intent"]["confirmation_phrase_matches"] is True
    assert approval["approval_gate"]["eligible_for_approval"] is True
    assert approval["approval_gate"]["approval_blockers"] == []
    assert approval["selected_action"]["action_id"] == action["action_id"]
    assert approval["selected_command"]["label"] == command["label"]
    assert approval["selected_command"]["command"] == command["command"]
    ...
    assert approval["policy"]["approval_does_not_execute_command"] is True
    assert approval["policy"]["command_still_requires_explicit_execution"] is True
    assert "# Operator Action Approval" in markdown
    assert "# Operator Action Approval" in md_path.read_text(encoding="utf-8")
...
def test_operator_action_execution_receipt_runs_only_approved_read_only_commands(
    ...
    repo = copy_repo_fixture(tmp_path)
    run_iteration_loop(
        run_id="operator-action-execution",
        max_rounds=1,
        repo_root=repo,
    )
    run_id = "operator-action-execution"
    ...
    action, read_only_command = next(
        (action, command)
        for action, command in action_command_pairs
    ...
    markdown = render_operator_action_execution_markdown(receipt)
    ...
    assert receipt["source_approval"]["approval_recorded"] is True
    assert receipt["selected_command"]["label"] == read_only_command["label"]
    ...
    assert receipt["mutation_guard"]["tracked_status_unchanged"] is True
    ...
    dangerous_approval = json.loads(approval_path.read_text(encoding="utf-8"))
    dangerous_approval["selected_command"].update(
        {
            "label": "start_next_iteration",
            "command": dangerous_command,
            "command_sha256": dangerous_sha,
            "computed_command_sha256": dangerous_sha,
            "command_sha256_matches": True,
            "expected_artifact": "manifest.json",
            "writes_repository": False,
            "promotes_champion": False,
            "runs_backtests": True,
    ...
    approval_path.write_text(
        json.dumps(dangerous_approval, indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
    )
    blocked = execute_operator_action_with_approval(
        run_id=run_id,
        approval_path=approval_path,
        experiments_dir=repo / "experiments",
        repo_root=repo,
    ...
    assert "command_runs_backtests" in blocked["evidence_checks"]["blockers"]
    assert "command_not_allowlisted" in blocked["evidence_checks"]["blockers"]
    assert blocked["command_execution"]["status"] == "blocked"
    assert blocked["policy"]["blocks_backtest_commands"] is True
...
def test_operator_action_audit_tracks_plan_approval_execution_chain(
    ...
    json_path, md_path, pending = write_operator_action_audit(
        run_dir=run_dir,
        experiments_dir=repo / "experiments",
        repo_root=repo,
    )
    assert pending["schema_version"] == OPERATOR_ACTION_AUDIT_SCHEMA_VERSION
    assert pending["status"] == "pending_approval"
    assert pending["ok"] is True
    assert pending["summary"]["approval_present"] is False
    assert pending["summary"]["execution_present"] is False
    assert pending["source_artifacts"]["action_plan"]["from_artifact"] is True
    ...
    _, _, ready = write_operator_action_audit(
        run_dir=run_dir,
        experiments_dir=repo / "experiments",
        repo_root=repo,
    )
    assert ready["status"] == "ready_for_execution"
    assert ready["summary"]["approval_recorded"] is True
    assert ready["selected_action"]["exists_in_plan"] is True
    ...
    json_path, _, completed = write_operator_action_audit(
        run_dir=run_dir,
        experiments_dir=repo / "experiments",
        repo_root=repo,
    )
    markdown = render_operator_action_audit_markdown(completed)
    built = build_operator_action_audit(
        run_dir=run_dir,
        experiments_dir=repo / "experiments",
        repo_root=repo,
    )
...
def test_operator_cockpit_aggregates_operator_views_without_authority(
    ...
    json_path.write_text(
        json.dumps(tampered_cockpit, indent=2, sort_keys=True),
        encoding="utf-8",
    )
    tampered_validation = validate_run_artifacts(
        run_id=run_id,
...
def test_operator_cockpit_report_flags_stale_source_snapshot(
    ...
    assert refresh["home_summary"]["next_command"].startswith(
    ...
    assert refresh["home_summary"]["next_command_boundary"] == (
        "operator_approval_receipt"
    )
    assert refresh["home_summary"]["next_command_writes_artifact"] == (
        "operator_action_approval.json"
    )
    assert (
        refresh["home_summary"][
            "next_command_requires_explicit_operator_invocation"
        ]
        is True
    )
    assert refresh["home_summary"]["next_command_records_operator_approval"] is True
    assert refresh["home_summary"]["next_command_uses_guarded_executor"] is False
    assert refresh["home_summary"]["next_command_is_hint_only"] is True
    ...
    assert "Next command boundary: `operator_approval_receipt`" in refresh_markdown
    assert (
        "Next command requires explicit invocation: `True`" in refresh_markdown
    )
...
def _minimal_operator_view_refresh_payload() -> dict[str, object]:
    ...
    return {
        ...
        "home_summary": {
            ...
            "next_command_requires_explicit_operator_invocation": True,
...
def test_iteration_loop_rejects_and_rolls_back_by_default(tmp_path: Path) -> None:
    ...
    config_change_candidate = json.loads(
    ...
    assert manifest["config_change_candidate"]["path"] == (
        "config_change_candidate.json"
    )
    assert manifest["config_change_candidate"]["markdown_path"] == (
        "config_change_candidate.md"
    )
    assert config_change_candidate["schema_version"] == (
        CONFIG_CHANGE_CANDIDATE_SCHEMA_VERSION
    )
    assert config_change_candidate["sources"][0]["file"]["path"].endswith(
        "memory_scope_recommendation.json"
    )
    assert config_change_candidate["summary"]["candidate_count"] == (
        manifest["config_change_candidate"]["candidate_count"]
    )
    assert config_change_candidate["policy"]["operator_must_apply_changes_manually"] is True
    assert "# Config Change Candidate" in config_change_markdown
    assert_matches_schema(
    ...
    assert manifest["operator_home"]["next_command_boundary"] == (
        "operator_approval_receipt"
    )
    assert manifest["operator_home"]["next_command_writes_artifact"] == (
        "operator_action_approval.json"
    )
    assert (
        manifest["operator_home"][
            "next_command_requires_explicit_operator_invocation"
        ]
        is True
    )
    assert (
        manifest["operator_home"]["next_command_requires_operator_approval"]
        is False
    )
    assert manifest["operator_home"]["next_command_records_operator_approval"] is True
    assert manifest["operator_home"]["next_command_uses_guarded_executor"] is False
    assert manifest["operator_home"]["next_command_is_hint_only"] is True
...
def test_artifact_validator_reports_iteration_diagnosis_selected_candidates_drift(
    ...
    repo = copy_repo_fixture(tmp_path)
    run_id = "artifact-diagnosis-selected-candidates-drift"
    run_iteration_loop(
        run_id=run_id,
        max_rounds=1,
        repo_root=repo,
    ...
    diagnosis_path = repo / "experiments" / run_id / "diagnosis.json"
    diagnosis = json.loads(diagnosis_path.read_text(encoding="utf-8"))
    assert diagnosis["selected_candidates"]
    diagnosis["selected_candidates"][0]["status"] = "wrong-status"
    diagnosis_path.write_text(
        json.dumps(diagnosis, indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
    )
    ...
    assert report["ok"] is False
    assert "diagnosis.json selected_candidates mismatch" in report["errors"]
...
def test_artifact_validator_reports_summary_config_application_dry_run_drift(
    tmp_path: Path,
) -> None:
    repo = copy_repo_fixture(tmp_path)
    run_id = "artifact-summary-config-application-dry-run-drift"
    run_iteration_loop(
        run_id=run_id,
        max_rounds=1,
        repo_root=repo,
    ...
    run_dir = repo / "experiments" / run_id
    manifest = json.loads((run_dir / "manifest.json").read_text(encoding="utf-8"))
    dry_run = manifest["config_application_dry_run"]
    summary_path = run_dir / "summary.md"
    summary_text = summary_path.read_text(encoding="utf-8")
    section_start = summary_text.index("## Config Application Dry Run")
    section_end = summary_text.index("\n## Config Lineage", section_start)
    dry_run_section = summary_text[section_start:section_end]
    dry_run_section = dry_run_section.replace(
        f"- Status: `{dry_run['status']}`",
        "- Status: `wrong_status`",
        1,
    )
    dry_run_section = dry_run_section.replace(
        f"- Artifact: `{dry_run['path']}`",
        "- Artifact: `wrong_config_application_dry_run.json`",
        1,
    )
    summary_text = (
        summary_text[:section_start] + dry_run_section + summary_text[section_end:]
    )
    summary_path.write_text(summary_text, encoding="utf-8")
    ...
    assert report["ok"] is False
    assert "summary.md config_application_dry_run status mismatch" in report[
        "errors"
    ]
    assert "summary.md config_application_dry_run path mismatch" in report["errors"]
...
def test_artifact_validator_reports_summary_run_closeout_drift(
    ...
    repo = copy_repo_fixture(tmp_path)
    run_id = "artifact-summary-run-closeout-drift"
    run_iteration_loop(
        run_id=run_id,
        max_rounds=1,
        repo_root=repo,
    ...
    run_dir = repo / "experiments" / run_id
    manifest = json.loads((run_dir / "manifest.json").read_text(encoding="utf-8"))
    closeout = manifest["run_closeout"]
    summary_path = run_dir / "summary.md"
    summary_text = summary_path.read_text(encoding="utf-8")
    section_start = summary_text.index("## Run Closeout")
    section_end = summary_text.index("\n## Operator Action Plan", section_start)
    closeout_section = summary_text[section_start:section_end]
    closeout_section = closeout_section.replace(
        f"- Status: `{closeout['status']}`",
        "- Status: `wrong_status`",
        1,
    )
    closeout_section = closeout_section.replace(
        f"- Markdown: `{closeout['markdown_path']}`",
        "- Markdown: `wrong_run_closeout.md`",
        1,
    )
    summary_text = (
        summary_text[:section_start] + closeout_section + summary_text[section_end:]
    )
    summary_path.write_text(summary_text, encoding="utf-8")
    ...
    assert report["ok"] is False
    assert "summary.md run_closeout status mismatch" in report["errors"]
    assert "summary.md run_closeout markdown_path mismatch" in report["errors"]


def test_artifact_validator_reports_summary_operator_action_plan_drift(
    tmp_path: Path,
) -> None:
    repo = copy_repo_fixture(tmp_path)
    run_id = "artifact-summary-action-plan-drift"
    run_iteration_loop(
        run_id=run_id,
        max_rounds=1,
        repo_root=repo,
    ...
    run_dir = repo / "experiments" / run_id
    manifest = json.loads((run_dir / "manifest.json").read_text(encoding="utf-8"))
    action_plan = manifest["operator_action_plan"]
    summary_path = run_dir / "summary.md"
    summary_text = summary_path.read_text(encoding="utf-8")
    section_start = summary_text.index("## Operator Action Plan")
    section_end = summary_text.index("\n## Operator Action Dashboard", section_start)
    action_plan_section = summary_text[section_start:section_end]
    action_plan_section = action_plan_section.replace(
        f"- Action count: `{action_plan['action_count']}`",
        "- Action count: `999`",
        1,
    )
    action_plan_section = action_plan_section.replace(
        f"- Markdown: `{action_plan['markdown_path']}`",
        "- Markdown: `wrong_operator_action_plan.md`",
        1,
    )
    summary_text = (
        summary_text[:section_start]
    ...
    assert report["ok"] is False
    assert "summary.md operator_action_plan action_count mismatch" in report[
        "errors"
    ]
    assert (
        "summary.md operator_action_plan markdown_path mismatch"
        in report["errors"]
    )
...
def test_artifact_validator_reports_summary_header_drift(tmp_path: Path) -> None:
    ...
    summary_text = summary_text.replace(
        f"- Final strategy commit: `{manifest['final_strategy_commit']}`",
        "- Final strategy commit: `wrong-commit`",
    )
    summary_path.write_text(summary_text, encoding="utf-8")
    ...
    assert report["ok"] is False
    assert "summary.md iteration header status mismatch" in report["errors"]
    assert (
        "summary.md iteration header final_strategy_commit mismatch"
        in report["errors"]
    )


def test_artifact_validator_reports_summary_dataset_drift(tmp_path: Path) -> None:
    repo = copy_repo_fixture(tmp_path)
    run_id = "artifact-summary-dataset-drift"
    run_iteration_loop(
        run_id=run_id,
        max_rounds=1,
        repo_root=repo,
    ...
    summary_text = summary_path.read_text(encoding="utf-8")
    summary_text = summary_text.replace(
        f"- validation: `{manifest['datasets']['validation']}`",
        "- validation: `data/train/sample_markets.csv`",
    )
    summary_path.write_text(summary_text, encoding="utf-8")
...
def test_artifact_validator_reports_summary_round_row_drift(
    ...
    repo = copy_repo_fixture(tmp_path)
    run_id = "artifact-summary-round-row-drift"
    run_iteration_loop(
        run_id=run_id,
        max_rounds=1,
        repo_root=repo,
...
def test_artifact_validator_reports_config_candidate_policy_violation(
    tmp_path: Path,
) -> None:
    repo = copy_repo_fixture(tmp_path)
    run_iteration_loop(
        run_id="config-candidate-policy-error",
        max_rounds=1,
        repo_root=repo,
    )
    path = (
        repo
        / "experiments/config-candidate-policy-error/config_change_candidate.json"
    )
    payload = json.loads(path.read_text(encoding="utf-8"))
    payload["policy"]["does_not_write_config"] = False
    path.write_text(json.dumps(payload, indent=2, sort_keys=True), encoding="utf-8")
...
def test_codex_cli_execution_preflight_blocks_operator_source_plan_drift(
    ...
    fake_codex = write_fake_command(
        tmp_path,
        "fake_codex_source_plan_drift.py",
        "#!/usr/bin/env python3\nprint('{}')\n",
    )
    request_path = write_operator_unlock_request_fixture(
        repo,
        repo / "operator_unlock_fixtures/source_plan_drift_request.json",
        run_id="source-plan-drift",
        executable=str(fake_codex),
        model="source-plan-drift-test",
        sandbox="workspace-write",
        workspace_root="workspaces",
    )
    request = json.loads(request_path.read_text(encoding="utf-8"))
    source_dry_run_path = repo / request["source_real_execution_dry_run"]["file"][
        "path"
    ]
    source_dry_run = json.loads(source_dry_run_path.read_text(encoding="utf-8"))
    source_dry_run["planned_execution"]["workspace_path"] = (
        "workspaces/source-plan-drift/codex_cli_real_execution/"
        "real_codex_execution/attempt_999_real_execution/strategy_workspace"
    )
    source_dry_run_path.write_text(
        json.dumps(source_dry_run, indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
    )
    request["source_real_execution_dry_run"]["file"] = file_record(
        source_dry_run_path,
    ...
    preflight = write_codex_cli_execution_preflight(
        output_path=repo
        / "experiments/source-plan-drift/codex_cli_execution_preflight.json",
        markdown_path=repo
        / "experiments/source-plan-drift/codex_cli_execution_preflight.md",
        run_dir=repo / "experiments/source-plan-drift",
        repo_root=repo,
        config=config,
    )
    ...
    assert profile["checks"]["operator_request_source_dry_run_plan_present"] is True
    assert (
        profile["checks"]["operator_request_source_dry_run_plan_matches_review"]
        is False
    )
    assert profile["checks"]["operator_request_workspace_path_matches_expected"] is True
    assert (
        "profile primary: operator_request_source_dry_run_plan_mismatch"
        in preflight["blocking_errors"]
    )
...
def test_artifact_validator_reports_operator_source_plan_mismatch(
    tmp_path: Path,
) -> None:
    repo = copy_repo_fixture(tmp_path)
    run_dir = repo / "experiments/operator-source-plan-drift"
    request_path = write_operator_unlock_request_fixture(
        repo,
        run_dir / "codex_cli_operator_unlock_request.json",
        run_id="operator-source-plan-drift",
    )
    request = json.loads(request_path.read_text(encoding="utf-8"))
    source_dry_run_path = repo / request["source_real_execution_dry_run"]["file"][
        "path"
    ...
    source_dry_run = json.loads(source_dry_run_path.read_text(encoding="utf-8"))
    source_dry_run["planned_execution"]["round_id"] = "codex_cli_shadow_execution"
    source_dry_run_path.write_text(
        json.dumps(source_dry_run, indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
    )
...
def test_experiment_list_and_show_helpers(tmp_path: Path) -> None:
    ...
    assert records[1]["operator_home"]["next_command_boundary"] == (
        "operator_approval_receipt"
    )
    assert records[1]["operator_home"]["next_command_writes_artifact"] == (
        "operator_action_approval.json"
    )
    assert (
        records[1]["operator_home"][
            "next_command_requires_explicit_operator_invocation"
        ]
        is True
    )
    assert records[1]["operator_home"]["next_command_records_operator_approval"] is True
    assert records[1]["operator_home"]["next_command_uses_guarded_executor"] is False
    assert records[1]["operator_home"]["next_command_is_hint_only"] is True
    assert records[1]["operator_next_command"]["available"] is True
    assert records[1]["operator_next_command"]["command_label"] == (
    ...
    assert records[1]["operator_next_command"]["selected_command_writes_artifact"] == (
        "operator_action_approval.json"
    )
    ...
    assert single["operator_home"]["next_command_boundary"] == ""  # type: ignore[index]
    ...
    assert iteration["operator_home"]["next_command_boundary"] == (  # type: ignore[index]
        "operator_approval_receipt"
    )
    assert iteration["operator_home"]["next_command_writes_artifact"] == (  # type: ignore[index]
        "operator_action_approval.json"
    )
    assert (  # type: ignore[index]
        iteration["operator_home"][
            "next_command_requires_explicit_operator_invocation"
        ]
        is True
    )
    assert iteration["operator_home"]["next_command_records_operator_approval"] is True  # type: ignore[index]
    assert iteration["operator_home"]["next_command_uses_guarded_executor"] is False  # type: ignore[index]
    assert iteration["operator_home"]["next_command_is_hint_only"] is True  # type: ignore[index]
    assert iteration["operator_next_command"]["available"] is True  # type: ignore[index]
    assert iteration["operator_next_command"]["command_label"] == (  # type: ignore[index]
        "review_operator_next_command"
    )
    ...
    assert iteration["operator_next_command"]["selected_command_writes_artifact"] == (  # type: ignore[index]
        "operator_action_approval.json"
    )
...
def test_experiment_summary_and_leaderboard_helpers(tmp_path: Path) -> None:
    ...
    assert summary["dashboard"]["operator_home_entry"]["next_command_boundary"] == (  # type: ignore[index]
        "operator_approval_receipt"
    )
    assert summary["dashboard"]["operator_home_entry"]["next_command_writes_artifact"] == (  # type: ignore[index]
        "operator_action_approval.json"
    )
    assert (  # type: ignore[index]
        summary["dashboard"]["operator_home_entry"][
            "next_command_requires_explicit_operator_invocation"
        ]
        is True
    )
    assert summary["dashboard"]["operator_home_entry"]["next_command_records_operator_approval"] is True  # type: ignore[index]
    assert summary["dashboard"]["operator_home_entry"]["next_command_uses_guarded_executor"] is False  # type: ignore[index]
    assert summary["dashboard"]["operator_home_entry"]["next_command_is_hint_only"] is True  # type: ignore[index]
    assert summary["dashboard"]["operator_next_command_entry"]["schema_version"] == (  # type: ignore[index]
        "experiment_operator_next_command_entry_v1"
    )
    assert summary["dashboard"]["operator_next_command_entry"]["available"] is True  # type: ignore[index]
    assert summary["dashboard"]["operator_next_command_entry"]["reason"] == "iteration_run"  # type: ignore[index]
    assert summary["dashboard"]["operator_next_command_entry"]["run_id"] == "iteration-rank"  # type: ignore[index]
    assert summary["dashboard"]["operator_next_command_entry"]["command_label"] == (  # type: ignore[index]
        "review_operator_next_command"
    )
    ...
    assert summary["dashboard"]["operator_next_command_entry"]["selected_command_boundary"] == (  # type: ignore[index]
        "operator_approval_receipt"
    )
    assert summary["dashboard"]["operator_next_command_entry"]["selected_command_writes_artifact"] == (  # type: ignore[index]
        "operator_action_approval.json"
    )
    ...
    assert (
        "Operator home next command boundary: `operator_approval_receipt`"
        in summary_markdown
    )
    assert (
        "Operator home next command requires explicit invocation: `True`"
        in summary_markdown
    )
    assert (
...
def _minimal_experiment_summary_dashboard_payload() -> dict[str, object]:
    return {
        ...
        "operator_home_entry": {
            ...
            "next_command_boundary": "operator_approval_receipt",
            "next_command_writes_artifact": "operator_action_approval.json",
            "next_command_requires_explicit_operator_invocation": True,
            "next_command_requires_operator_approval": False,
            "next_command_records_operator_approval": True,
            "next_command_uses_guarded_executor": False,
            "next_command_is_hint_only": True,
        ...
        "operator_next_command_entry": {
            ...
            "selected_command_status": "blocked_by_home_blockers",
            "selected_command_boundary": "operator_approval_receipt",
            "selected_command_writes_artifact": "operator_action_approval.json",
            "selected_command_requires_explicit_operator_invocation": True,
            "selected_command_requires_operator_approval": False,
            "selected_command_records_operator_approval": True,
...
def test_experiment_summary_dashboard_validation_reports_counter_drift() -> None:
    ...
    operator_next_command["run_kind"] = "single_run"
    ...
    operator_next_command["selection_source"] = "manual"
    operator_next_command["selected_command_status"] = "ready"
    operator_next_command["selected_command"] = "python -m orchestrator.run_loop"
    operator_next_command["selected_command_boundary"] = "mutation"
    ...
    assert "experiment_summary_dashboard operator_home next command missing" in errors
    assert "experiment_summary_dashboard operator_home next boundary missing" in errors
    assert (
    ...
    assert "experiment_summary_dashboard operator_next_command boundary mismatch" in errors
    assert "experiment_summary_dashboard operator_next_command write mismatch" in errors
    ...
    assert "experiment_summary_dashboard operator_next_command source mismatch" in errors
...
def test_experiments_cli_list_and_show_work(tmp_path: Path) -> None:
    ...
    assert list_payload[0]["operator_next_command"]["available"] is False
    assert list_payload[0]["operator_next_command"]["reason"] == "not_iteration_run"
    ...
    assert list_payload[1]["operator_home"]["next_command"].startswith(
        "python -m orchestrator.operator_action_approval "
    )
    assert list_payload[1]["operator_home"]["next_command_boundary"] == (
        "operator_approval_receipt"
    )
    assert list_payload[1]["operator_next_command"]["available"] is True
    assert list_payload[1]["operator_next_command"]["command"] == (
        "python -m orchestrator.experiments next-command "
        "cli-list-iteration --markdown"
    )
    assert list_payload[1]["operator_next_command"]["selected_command"] == (
        list_payload[1]["operator_home"]["next_command"]
    )
    assert list_payload[1]["operator_next_command"]["selected_command_boundary"] == (
        "operator_approval_receipt"
    )
    ...
    assert show_payload["operator_next_command"]["available"] is False
    assert show_payload["operator_next_command"]["reason"] == "not_iteration_run"
    assert show_payload["operator_next_command"]["command"] == ""
    ...
    assert show_iteration_payload["operator_home"]["next_command"].startswith(
        "python -m orchestrator.operator_action_approval "
    )
    assert show_iteration_payload["operator_home"]["next_command_writes_artifact"] == (
        "operator_action_approval.json"
    )
    assert show_iteration_payload["operator_next_command"]["available"] is True
    assert show_iteration_payload["operator_next_command"]["command"] == (
        "python -m orchestrator.experiments next-command "
        "cli-list-iteration --markdown"
    )
    assert show_iteration_payload["operator_next_command"]["selected_command"] == (
        show_iteration_payload["operator_home"]["next_command"]
    )
    assert show_iteration_payload["operator_next_command"]["selected_command_status"] == (
        "blocked_by_home_blockers"
    )
...
def test_experiments_cli_summary_and_leaderboard_work(tmp_path: Path) -> None:
    ...
    assert summary["dashboard"]["operator_home_entry"]["next_command"] == ""
    assert summary["dashboard"]["operator_next_command_entry"]["available"] is False
    assert summary["dashboard"]["operator_next_command_entry"]["reason"] == (
        "not_iteration_run"
    )
    assert summary["dashboard"]["operator_next_command_entry"]["run_id"] == (
        "cli-summary"
    )
    assert summary["dashboard"]["operator_next_command_entry"]["command"] == ""
    assert summary["dashboard"]["operator_next_command_entry"]["selected_command"] == ""
    assert (
        summary["dashboard"]["operator_next_command_entry"][
    ...
    assert "Operator next-command selector command: `unavailable`" in (
        markdown_result.stdout
    )
    assert "Operator next-command selected status: `unavailable`" in (
        markdown_result.stdout
    )
...
def test_experiments_candidate_leaderboard_helpers_and_cli_work(
    ...
    config_candidate = config_change_candidate_report(
        run_id="cli-candidates",
        experiments_dir=repo / "experiments",
    )
    ...
    write_operator_action_approval(
        ...
        action_id=action["action_id"],
        command_label=command["label"],
        operator_id="cli-test",
        explicit_approval=True,
        confirmation_phrase=OPERATOR_ACTION_CONFIRMATION_PHRASE,
    )
    execute_operator_action_with_approval(
        run_id="cli-candidates",
        approval_path=repo / "experiments/cli-candidates/operator_action_approval.json",
        experiments_dir=repo / "experiments",
        repo_root=repo,
    )
    action_approval = operator_action_approval_report(
        run_id="cli-candidates",
        experiments_dir=repo / "experiments",
    )
    action_approval_markdown = render_operator_action_approval_markdown(
        action_approval
    )
    dynamic_action_approval = operator_action_approval_report(
        run_id="cli-candidates",
        experiments_dir=repo / "experiments",
        action_id=action["action_id"],
        command_label=command["label"],
    )
    action_execution = operator_action_execution_report(
        run_id="cli-candidates",
        experiments_dir=repo / "experiments",
    )
    action_execution_markdown = render_operator_action_execution_markdown(
        action_execution
    )
    write_operator_action_audit(
        run_dir=repo / "experiments/cli-candidates",
    ...
    action_audit = operator_action_audit_report(
        run_id="cli-candidates",
        experiments_dir=repo / "experiments",
    )
    ...
    action_approval_markdown_result = subprocess.run(
        [
            sys.executable,
            "-m",
            "orchestrator.experiments",
    ...
    action_execution_result = subprocess.run(
        [
            sys.executable,
            "-m",
            "orchestrator.experiments",
    ...
    action_execution_markdown_result = subprocess.run(
        [
            sys.executable,
            "-m",
            "orchestrator.experiments",
    ...
    action_audit_markdown_result = subprocess.run(
        [
            sys.executable,
            "-m",
            "orchestrator.experiments",
    ...
    assert action_approval["status"] == "approval_recorded"
    assert action_approval["operator_intent"]["approval_recorded"] is True
    ...
    assert action_execution["status"] == "completed"
    ...
    assert action_audit["status"] == "execution_completed"
    ...
    assert (
        action_approval_payload["schema_version"]
        == OPERATOR_ACTION_APPROVAL_SCHEMA_VERSION
    )
    assert action_approval_payload["from_artifact"] is True
    assert action_approval_payload["status"] == "approval_recorded"
    ...
    assert (
        action_execution_payload["schema_version"]
        == OPERATOR_ACTION_EXECUTION_RECEIPT_SCHEMA_VERSION
    )
    assert action_execution_payload["from_artifact"] is True
    assert action_execution_payload["status"] == "completed"
    assert action_execution_payload["command_execution"]["status"] == "completed"
    assert_matches_schema_payload(
    ...
    assert config_candidate_payload["from_artifact"] is True
